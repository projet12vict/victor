...
const LoginScreen: React.FC<{
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (company && !company.isActive) {
            setError('Esta empresa está suspensa. Contacte o administrador.');
            return;
        }

        // Authentication Logic
        const user = users.find(u => {
            // If logging into a specific company, check companyId match
            if (company) {
                return u.companyId === company.id && 
                       u.email.toLowerCase() === email.toLowerCase() &&
                       u.password === password;
            } 
            // If logging in as Super Admin (Global), check for SUPER_ADMIN role
            else {
    ...
    return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
                ...
                <div className="absolute top-40 right-20 w-72 h-72 bg-purple-500 rounded-full blur-3xl"></div>
            </div>
            ...
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden z-10 animate-in fade-in zoom-in duration-300">
                <div className="p-8 pb-6 bg-slate-50 border-b border-slate-100 flex flex-col items-center">
                    <div className="w-24 h-24 rounded-full bg-white shadow-md mb-4 flex items-center justify-center overflow-hidden border-4 border-slate-100">
                        {company ? (
                            company.logoUrl ? (
                                <img src={company.logoUrl} alt={company.name} className="w-full h-full object-cover" />
                            ) : (
                                <Store className="text-brand-500" size={40} />
                            )
                        ) : (
                            <Logo size="sm" />
                        )}
                    ...
                    <h2 className="text-xl font-bold text-slate-800 text-center">
                        {company ? company.name : 'Administração Global'}
                    </h2>
                    <p className="text-sm text-slate-500">
                        {company ? 'Acesso Restrito' : 'Portal do Super Admin'}
                    </p>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-5">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
                                placeholder={company ? "ex: gerente@empresa.com" : "victorallissson@gmail.com"}
                            />
                        </div>
                    </div>
                    ...
                    <div className="pt-2">
                        ...
                        >
                            {company ? 'Entrar na Empresa' : 'Aceder Painel'}
                        </button>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-200 flex flex-col max-h-[90vh]">
                <div className="p-6 bg-slate-900 text-white text-center shrink-0">
                    <h2 className="text-xl font-bold">Pagamento - Mesa {tableNumber}</h2>
                    <div className="flex justify-center items-baseline gap-2 mt-2">
                        <span className="text-3xl font-bold text-brand-400">{formatCurrency(effectiveTotal)}</span>
                        {billType === 'INDIVIDUAL' && <span className="text-sm text-slate-400">(Selecionado)</span>}
                    </div>
                </div>

                <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
                    {/* Left Column: Items Selection */}
                    <div className="flex-1 p-6 border-r border-slate-100 overflow-y-auto bg-slate-50">
                        <div className="flex bg-slate-200 p-1 rounded-lg mb-4">
                            <button 
                                onClick={() => setBillType('JOINT')}
                                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${billType === 'JOINT' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Conta Total
                            </button>
                            <button 
                                onClick={() => setBillType('INDIVIDUAL')}
                                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all flex items-center justify-center gap-2 ${billType === 'INDIVIDUAL' ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                <UserIcon size={14} /> Dividir / Individual
                            </button>
                        </div>

                        <h3 className="font-bold text-slate-700 mb-3 text-sm uppercase">Itens a Pagar</h3>
                        <div className="space-y-2">
                            {unpaidItems.length === 0 ? (
                                <p className="text-slate-400 italic text-sm">Todos os itens desta mesa já foram pagos.</p>
                            ) : (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            {selectedIndices.includes(item.originalIndex) ? 
                                                ...
                                                <SquareIcon className="text-slate-300" size={20} />
                                            }
                                            <div>
                                                <div className="font-medium text-slate-800 text-sm">{item.product.name}</div>
                                                <div className="text-xs text-slate-500">{item.qty}x un.</div>
                                            </div>
                                        </div>
                    ...
                    <div className="flex-1 p-6 space-y-6 overflow-y-auto">
                        <div className="grid grid-cols-2 gap-4">
                            <button 
                                ...
                                className={`p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${method === 'CARD' ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                            >
                                <CreditCard size={32} />
                            ...
                            <button 
                                ...
                                className={`p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${method === 'CASH' ? 'border-green-500 bg-green-50 text-green-700' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                            >
                                <Banknote size={32} />
                        ...
                        {method === 'CASH' && (
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4 animate-in slide-in-from-top-2">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Valor Entregue</label>
                                    <div className="relative">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold">$</span>
                                        <input 
                                            type="number" 
                                            autoFocus
                                            className="w-full pl-8 pr-4 py-3 text-lg font-bold border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none"
                                            placeholder="0.00"
                                            value={cashReceived}
                                ...
                                <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                                    <span className="text-slate-600 font-medium">Troco a devolver:</span>
                        ...
                        <div className="pt-4">
                            ...
                            <button 
                                onClick={onClose}
                                className="w-full mt-3 py-3 border border-slate-300 rounded-xl font-medium text-slate-700 hover:bg-slate-50"
                            >
                                Cancelar
                            </button>
                        </div>
...
const Sidebar: React.FC<{
    ...
    return (
        <>
            ...
            {/* Sidebar Container */}
            <aside className={`fixed top-0 left-0 h-full w-64 bg-slate-900 text-slate-300 flex flex-col shadow-2xl z-50 transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                <div className="p-6 border-b border-slate-800 flex items-center justify-center relative">
                    {/* Close button for mobile */}
                    <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 lg:hidden text-slate-500 hover:text-white">
                        <X size={20} />
                    </button>
                    ...
                    {activeCompany ? (
                        <div className="flex flex-col items-center gap-2">
                            ...
                            <span className="font-bold text-white text-center leading-tight">{activeCompany.name}</span>
                        </div>
                    ) : (
                        <Logo size="sm" invert />
                    )}
                ...
                <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
                    {role === UserRole.SUPER_ADMIN && (
                        <>
                            <div className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-500 mt-2">
                                Administração Global
                            </div>
                            <button
                                onClick={() => { setView('SUPER_ADMIN'); setIsOpen(false); }}
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'SUPER_ADMIN' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                            >
                                <Building2 size={20} />
                            ...
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'SUPER_ADMIN_CONFIG' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                            >
                                <ShieldCheck size={20} />
                    ...
                    {activeCompany && (
                        <>
                            ...
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'DASHBOARD' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                            >
                            ...
                            {(activeCompany.type === CompanyType.RESTAURANT || activeCompany.type === CompanyType.MIXED) && (
                                <>
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'RESTAURANT_POS' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                                    >
                                        <UtensilsCrossed size={20} />
                                        <span className="font-medium">Sala / Mesas</span>
                                    </button>
                                    <button
                                        onClick={() => { setView('KITCHEN'); setIsOpen(false); }}
                                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'KITCHEN' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                                    >
                                        <ChefHat size={20} />
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'BAR' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                                    >
                                        <Wine size={20} />
                                        <span className="font-medium">Bar / Copa</span>
                                    </button>
                                </>
                            )}
                            ...
                            {(activeCompany.type === CompanyType.SUPERMARKET || activeCompany.type === CompanyType.MIXED) && (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'SUPERMARKET_POS' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                                >
                                    <ShoppingCart size={20} />
                                    <span className="font-medium">Caixa / POS</span>
                                </button>
                            )}

                            <button
                                onClick={() => { setView('SETTINGS'); setIsOpen(false); }}
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${view === 'SETTINGS' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}`}
                            >
                                <Settings size={20} />
                ...
                <div className="p-4 border-t border-slate-800">
                    {currentUser ? (
                        ...
                        <div className="flex items-center gap-3 mb-4 px-2">
                            <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold">S</div>
                            <div className="overflow-hidden">
                    ...
                    <button 
                        onClick={onLogout}
                        className="w-full flex items-center gap-2 justify-center px-4 py-2 bg-slate-800 hover:bg-red-900/30 hover:text-red-400 text-slate-400 rounded-lg transition-colors text-sm font-medium"
                    >
                        <LogOut size={16} />
                        <span>Sair</span>
                    </button>
    ...
    return (
        <div className="p-8 h-full overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-slate-800">Gestão de Empresas</h1>
                <button onClick={() => setIsCreating(true)} className="bg-brand-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
            ...
            {isCreating && (
                <div className="bg-white p-6 rounded-xl shadow-md mb-8 border border-slate-200 animate-in fade-in slide-in-from-top-4">
                    <h3 className="font-bold text-lg mb-4">Nova Empresa</h3>
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input placeholder="Nome da Empresa" value={name} onChange={e => setName(e.target.value)} className="border p-2 rounded" required />
                        <select value={type} onChange={e => setType(e.target.value as CompanyType)} className="border p-2 rounded">
                        ...
                        <input placeholder="URL do Logo" value={logoUrl} onChange={e => setLogoUrl(e.target.value)} className="border p-2 rounded" />
                        <input placeholder="Código de Ativação" value={activationCode} onChange={e => setActivationCode(e.target.value)} className="border p-2 rounded" required />
                        
                        <div className="col-span-full mt-2 border-t pt-4">
                            <h4 className="text-sm font-bold text-slate-500 mb-2">Administrador da Empresa</h4>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <input placeholder="Nome Admin" value={adminName} onChange={e => setAdminName(e.target.value)} className="border p-2 rounded" required />
                                <input placeholder="Email Admin" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} className="border p-2 rounded" type="email" required />
                                <input placeholder="Senha Admin" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} className="border p-2 rounded" type="password" required />
                            </div>
                        </div>

                        <div className="col-span-full flex justify-end gap-2 mt-4">
                            <button type="button" onClick={() => setIsCreating(false)} className="px-4 py-2 text-slate-600">Cancelar</button>
                            <button type="submit" className="bg-brand-600 text-white px-4 py-2 rounded">Criar Empresa</button>
                        </div>
                    </form>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {companies.map(company => (
                    <div key={company.id} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col">
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <span className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full">{company.type}</span>
                            </div>
                        </div>
                        <div className="mt-auto flex justify-between items-center pt-4 border-t border-slate-100">
                            <button onClick={() => onSelectCompany(company.id)} className="text-brand-600 font-bold text-sm hover:underline">Aceder</button>
                            <div className="flex gap-2">
                                <button onClick={() => onToggleStatus(company.id)} title={company.isActive ? "Suspender" : "Ativar"} className={`${company.isActive ? 'text-green-500' : 'text-red-500'}`}>
                                    <Power size={18} />
                                </button>
...
const SuperAdminConfig: React.FC<{
    ...
    return (
        <div className="p-8 h-full overflow-y-auto">
            <h1 className="text-3xl font-bold text-slate-800 mb-8">Configurações Globais</h1>
            
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Key size={24} /> Códigos de Ativação</h2>
                <div className="flex gap-4 mb-6">
                    <button onClick={() => onCreateCode('BASIC')} className="px-4 py-2 bg-slate-200 hover:bg-slate-300 rounded-lg font-medium text-slate-800">
                        Gerar Código Basic
                    </button>
                    <button onClick={() => onCreateCode('PREMIUM')} className="px-4 py-2 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 rounded-lg font-medium">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {codes.map(code => (
                                <tr key={code.id}>
                                    ...
                                    <td className="p-3"><span className={`px-2 py-1 rounded text-xs font-bold ${code.plan === 'PREMIUM' ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-100 text-slate-600'}`}>{code.plan}</span></td>
                                    <td className="p-3">
                                        {code.status === 'USED' ? <span className="text-red-500 font-bold">Usado</span> : <span className="text-green-500 font-bold">Disponível</span>}
                                    </td>
                                    <td className="p-3 text-slate-500">{new Date(code.generatedAt).toLocaleDateString()}</td>
                                    <td className="p-3">
                                        <button onClick={() => onDeleteCode(code.id)} className="text-slate-400 hover:text-red-600"><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                            {codes.length === 0 && <tr><td colSpan={5} className="p-4 text-center text-slate-400">Nenhum código gerado</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    return (
        <div className="p-8 h-full overflow-y-auto">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-96">
                <h3 className="text-lg font-bold text-slate-800 mb-4">Vendas da Semana</h3>
    ...
    return (
        <div className="flex h-full overflow-hidden">
            ...
            <div className="flex-1 p-6 bg-slate-100 overflow-y-auto">
                <h1 className="text-2xl font-bold text-slate-800 mb-6">Sala / Mesas</h1>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {tables.map(table => {
                         let statusColor = 'bg-white border-slate-200 text-slate-600';
                         if (table.status === TableStatus.OCCUPIED) statusColor = 'bg-red-50 border-red-200 text-red-600';
                         if (table.status === TableStatus.PAYMENT) statusColor = 'bg-yellow-50 border-yellow-200 text-yellow-600';
                         if (table.status === TableStatus.RESERVED) statusColor = 'bg-purple-50 border-purple-200 text-purple-600';
                         ...
                         return (
                            <button 
                                ...
                                className={`p-4 rounded-xl border-2 shadow-sm flex flex-col justify-between aspect-square transition-all hover:shadow-md ${statusColor} ${selectedTableId === table.id ? 'ring-2 ring-brand-500' : ''}`}
                            >
                                <div className="flex justify-between items-start w-full">
                                    <span className="font-bold text-xl">Mesa {table.number}</span>
                                    <span className="text-xs font-bold uppercase opacity-70">{table.status}</span>
                                </div>
                                {order && (
                                    <div className="mt-auto text-right">
                                        <div className="text-xs opacity-70">{order.items.length} itens</div>
            ...
            {selectedTableId && (
                <div className="w-96 bg-white border-l border-slate-200 flex flex-col h-full shadow-xl z-10">
                    ...
                    <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                        <div>
                            <h2 className="font-bold text-lg text-slate-800">Mesa {tables.find(t => t.id === selectedTableId)?.number}</h2>
                            <span className="text-xs text-slate-500">Pedido #{selectedTableId}99</span>
                        </div>
                        <button onClick={() => setSelectedTableId(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
                    </div>

                    {/* Order Items */}
                    <div className="flex-1 overflow-y-auto p-4">
                        {!activeOrder ? (
                            <div className="text-center text-slate-400 mt-10">Mesa Livre<br/>Adicione itens para abrir pedido</div>
                        ) : (
                                ...
                                {activeOrder.items.map((item, idx) => (
                                    <div key={idx} className={`flex justify-between items-start pb-2 border-b border-slate-50 ${item.paid ? 'opacity-50 grayscale' : ''}`}>
                                        <div className="flex-1">
                                            ...
                                            <div className="text-xs text-slate-500">{formatCurrency(item.product.price)} x {item.qty}</div>
                                        </div>
                                        <div className="font-bold text-sm text-slate-700">{formatCurrency(item.product.price * item.qty)}</div>
                                        {item.paid && <CheckCircle size={14} className="ml-2 text-green-500" />}
                                    </div>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="p-4 bg-slate-50 border-t border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-2">
                             ...
                             <button 
                                ...
                                className="bg-orange-100 text-orange-700 py-2 rounded-lg font-bold text-sm hover:bg-orange-200 disabled:opacity-50"
                             >
                                 Enviar Cozinha
                             </button>
                             <button 
                                ...
                                disabled={!activeOrder}
                                className="bg-green-600 text-white py-2 rounded-lg font-bold text-sm hover:bg-green-700 disabled:opacity-50"
                             >
                                 Pagamento
                             </button>
                        </div>
                    ...
                    <div className="h-1/3 border-t border-slate-200 flex flex-col">
                        <div className="p-2 border-b flex gap-2 overflow-x-auto">
                            {categories.map(c => (
                                <button 
                                    ...
                                    className={`px-3 py-1 rounded-full text-xs whitespace-nowrap ${selectedCategory === c ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-600'}`}
                                >{c}</button>
                            ))}
                        </div>
                        <div className="flex-1 overflow-y-auto p-2 grid grid-cols-3 gap-2 bg-slate-50">
                            {filteredProducts.map(p => (
                                ...
                                >
                                    <div className="text-[10px] font-bold line-clamp-2">{p.name}</div>
                                    <div className="text-[10px] text-brand-600">{p.price}</div>
                                </button>
                            ))}
                        </div>
                    </div>
        ...
        <div className="flex h-full">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {filteredProducts.map(p => (
                        <button 
                            key={p.id}
                            onClick={() => addToCart(p)}
                            className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-all text-left group"
                        >
                            <div className="font-bold text-slate-800 group-hover:text-brand-600 transition-colors">{p.name}</div>
            ...
            <div className="w-96 bg-white shadow-2xl flex flex-col z-20">
                <div className="p-6 bg-slate-900 text-white">
                    <h2 className="text-2xl font-bold flex items-center gap-2"><ShoppingCart /> Caixa</h2>
                </div>
                        ...
                        cart.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
                                <div>
                                    ...
                                    <div className="text-sm text-slate-500">{item.qty} x {formatCurrency(item.product.price)}</div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="font-bold">{formatCurrency(item.product.price * item.qty)}</span>
    ...
    return (
        <div className="p-8 h-full overflow-y-auto">
            ...
            <div className="flex gap-4 mb-8 border-b border-slate-200 pb-1">
                <button onClick={() => setTab('GENERAL')} className={`pb-3 px-2 font-medium transition-colors ${tab === 'GENERAL' ? 'border-b-2 border-brand-600 text-brand-600' : 'text-slate-500'}`}>Geral</button>
                <button onClick={() => setTab('PRODUCTS')} className={`pb-3 px-2 font-medium transition-colors ${tab === 'PRODUCTS' ? 'border-b-2 border-brand-600 text-brand-600' : 'text-slate-500'}`}>Produtos</button>
                <button onClick={() => setTab('USERS')} className={`pb-3 px-2 font-medium transition-colors ${tab === 'USERS' ? 'border-b-2 border-brand-600 text-brand-600' : 'text-slate-500'}`}>Utilizadores</button>
            </div>

            {tab === 'GENERAL' && (
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 max-w-2xl">
                    <h3 className="font-bold text-lg mb-4">Dados da Empresa</h3>
            ...
            {tab === 'PRODUCTS' && (
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                    <div className="flex gap-4 mb-6 p-4 bg-slate-50 rounded-lg">
                        <input placeholder="Nome Produto" value={newProdName} onChange={e => setNewProdName(e.target.value)} className="border p-2 rounded" />
                        <input placeholder="Preço" type="number" value={newProdPrice} onChange={e => setNewProdPrice(e.target.value)} className="border p-2 rounded w-24" />
                        <button onClick={handleAddProduct} className="bg-brand-600 text-white px-4 py-2 rounded font-bold">Adicionar</button>
                    </div>
                        ...
                        <tbody>
                            {products.map(p => (
                                <tr key={p.id} className="border-b last:border-0">
                                    ...
                                    <td className="py-2">
                                        <button onClick={() => setProducts(prev => prev.filter(x => x.id !== p.id))} className="text-red-500"><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {tab === 'USERS' && (
                 <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                    <p className="text-slate-500 mb-4">Gestão de utilizadores (Exibição apenas)</p>
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b">
                                <th className="pb-2">Nome</th>
                        ...
                        <tbody>
                            {users.map(u => (
                                <tr key={u.id} className="border-b last:border-0">
                                    ...
                                    <td className="py-2"><span className="bg-slate-100 px-2 py-1 rounded text-xs">{u.role}</span></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
    ...
    return (
        <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
            ...
            <main className={`flex-1 h-full overflow-hidden relative transition-all duration-300 ${view !== 'LOGIN' ? 'lg:ml-64' : ''} w-full`}>
                ...
                {view === 'KITCHEN' && (
                     <div className="p-8 h-full bg-slate-200 overflow-y-auto">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                ...
                                .map(([tableId, order]) => {
                                    ...
                                    <div key={tableId} className={`bg-white rounded-lg shadow-md border-l-8 overflow-hidden flex flex-col ${order.kitchenStatus === OrderStatus.READY ? 'border-green-500' : 'border-yellow-500'}`}>
                                        <div className={`p-4 border-b border-slate-100 flex justify-between items-center ${order.kitchenStatus === OrderStatus.READY ? 'bg-green-50' : 'bg-yellow-50'}`}>
                                            <span className="font-bold text-lg">Mesa {tables.find(t => t.id === Number(tableId))?.number}</span>
                                            <span className="text-sm font-mono">{order.startTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                        </div>
                                        
                                        <div className="p-4 space-y-4 flex-1">
                                            {/* Kitchen Items Section */}
                                            <div>
                                                <h4 className="text-xs font-bold text-orange-600 uppercase flex items-center gap-1 mb-2 border-b border-orange-100 pb-1">
                                                    ...
                                                    {kitchenItems.map((item, i) => (
                                                        <div key={i} className="flex flex-col border-b border-slate-50 last:border-0 pb-1 last:pb-0">
                                                            <div className="flex justify-between text-sm">
                                                                <span className="font-medium text-slate-800">{item.qty}x {item.product.name}</span>
                                                            </div>
                                                            {item.observation && (
                                                                <span className="text-xs text-red-500 font-bold italic pl-2 border-l-2 border-red-200">
                            ...
                            {(Object.entries(tableOrders) as [string, ActiveOrder][]).filter(([_, order]) => order.kitchenStatus !== OrderStatus.DELIVERED && order.items.some(i => !['Drinks', 'Bebidas'].includes(i.product.category))).length === 0 && (
                                <div className="col-span-full flex flex-col items-center justify-center text-slate-400 py-12">
                                    ...
                                    <p className="text-xl">Cozinha livre!</p>
                                </div>
                            )}
                         </div>
                     </div>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                ...
                                .map(([tableId, order]) => {
                                    ...
                                    <div key={tableId} className={`bg-white rounded-lg shadow-md border-l-8 overflow-hidden flex flex-col ${order.barStatus === OrderStatus.READY ? 'border-green-500' : 'border-blue-500'}`}>
                                        ...
                                        <div className="p-4 space-y-4 flex-1">
                                            {/* Bar Items Section */}
                                            <div>
                                                <h4 className="text-xs font-bold text-blue-600 uppercase flex items-center gap-1 mb-2 border-b border-blue-100 pb-1">
                            ...
                            {(Object.entries(tableOrders) as [string, ActiveOrder][]).filter(([_, order]) => order.barStatus !== OrderStatus.DELIVERED && order.items.some(i => ['Drinks', 'Bebidas'].includes(i.product.category))).length === 0 && (
                                <div className="col-span-full flex flex-col items-center justify-center text-slate-400 py-12">
                                    ...
                                    <p className="text-xl">Bar livre!</p>
                                </div>
                            )}
                         </div>
                     </div>
